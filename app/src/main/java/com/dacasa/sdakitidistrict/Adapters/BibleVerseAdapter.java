...
public class BibleVerseAdapter extends RecyclerView.Adapter<BibleVerseAdapter.VerseHolder> {
    ...
    public void onBindViewHolder(VerseHolder holder, int position) {
        ...
        if (nightMode){
            holder.verse_no.setTextColor(context.getResources().getColor(R.color.black));
        }else {
            holder.verse_no.setTextColor(Color.BLACK);
        }
        holder.verse.setTextColor(selection.contains(verse) ? Color.parseColor("#FF9900") : !nightMode ? Color.BLACK : context.getResources().getColor(R.color.black));
